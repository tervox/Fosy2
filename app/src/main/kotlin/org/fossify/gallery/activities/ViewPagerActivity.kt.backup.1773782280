...
class ViewPagerActivity : BaseViewerActivity(), ViewPager.OnPageChangeListener, ViewPagerFragment.FragmentListener {
    ...
    private fun copyMoveTo(isCopyOperation: Boolean) {
        ...
        // Pausa o vídeo antes de mover para evitar travamento
        if (!isCopyOperation) {
        ...
        tryCopyMoveFilesTo(fileDirItems, isCopyOperation) {
            ...
            if (!isCopyOperation) {
                ...
                // Avança direto para o próximo sem esperar refresh
                runOnUiThread {
                    val nextPos = if (mPos < mMediaFiles.lastIndex) mPos else maxOf(0, mPos - 1)
                    mMediaFiles.removeAt(mPos)
                    if (mMediaFiles.isEmpty()) {
                        finish()
                    } else {
                        mPos = nextPos.coerceAtMost(mMediaFiles.lastIndex)
