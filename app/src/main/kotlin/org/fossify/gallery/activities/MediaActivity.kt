...
class MediaActivity : SimpleActivity(), MediaOperationsListener {
    ...
    private var mPath = ""
    private var mDirName = ""
    private var mFolderSizeStr = ""
    ...
    private fun tryLoadGallery() {
        requestMediaPermissions {
            ...
            val searchHint = if (mShowAll) {
                ...
                getString(org.fossify.commons.R.string.search_in_placeholder, mDirName)
            }
            ...
            // Restaura tamanho já calculado antes (persiste entre sessões via prefs)
            val cachedSize = if (!mShowAll) config.getCachedFolderSize(mPath) else ""
            if (cachedSize.isNotEmpty()) mFolderSizeStr = cachedSize

            val titleName = when {
                mShowAll -> getString(R.string.all_folders)
                else -> mDirName
            }
            updateFolderTitle(titleName, if (!mShowAll) mFolderSizeStr else "")
    ...
    private fun updateFolderTitle(name: String, size: String) {
        if (size.isEmpty() || !config.showFolderSize) {
            binding.mediaMenu.requireToolbar().title = name
            binding.mediaMenu.requireToolbar().subtitle = null
        } else {
            binding.mediaMenu.requireToolbar().title = name
            binding.mediaMenu.requireToolbar().subtitle = size
        }
    }
    ...
    private fun getMedia() {
        ...
        // Sempre mostra o cache do BD primeiro (rápido), depois atualiza em background
        getCachedMedia(
            mPath,
    ...
    private fun gotMedia(media: ArrayList<ThumbnailItem>, isFromCache: Boolean) {
        ...
        runOnUiThread {
            ...
            // Mostra tamanho total da pasta no título
            if (!mShowAll && media.isNotEmpty() && config.showFolderSize) {
                val totalSize = media.filterIsInstance<Medium>().sumOf { it.size }
                if (totalSize > 0 && mDirName.isNotEmpty()) {
                    mFolderSizeStr = totalSize.formatSize()
                    config.setCachedFolderSize(mPath, mFolderSizeStr)
                    updateFolderTitle(mDirName, mFolderSizeStr)
                }
            }
        }
        ...
        // MMR para vídeos sem duração — roda sempre (cache e live)
        if (config.showThumbnailVideoDuration) {
            val allVideos = mMedia.filterIsInstance<Medium>().filter { it.isVideo() && it.videoDuration == 0 }
            if (allVideos.isNotEmpty()) {
                Thread {
                    allVideos.forEach { medium ->
                        try {
                            ...
                            if (durationSec > 0) {
                                medium.videoDuration = durationSec
                                try { mediaDB.updateVideoDuration(medium.path, durationSec) } catch (_: Exception) {}
                                runOnUiThread {
                                    val idx = mMedia.indexOfFirst { it is Medium && (it as Medium).path == medium.path }
                                    if (idx >= 0) getMediaAdapter()?.notifyItemChanged(idx)
                                }
                            }
                        } catch (_: Exception) {}
                    }
        ...
        if (!isFromCache) {
            ...
            Thread {
                try {
                    // 1. Carrega durações salvas no BD (UMA query) para preservar antes do REPLACE
                    val savedDurations: Map<String, Int> = try {
                        mediaDB.getVideoDurationsMap().associate { it.path to it.duration }
                    } catch (_: Exception) { emptyMap() }

                    // 2. Restaura duração calculada nos itens que MediaStore retornou como 0
                    if (savedDurations.isNotEmpty()) {
                        mediaToInsert.forEach { medium ->
                            if (medium.isVideo() && medium.videoDuration == 0) {
                                val saved = savedDurations[medium.path] ?: 0
                                if (saved > 0) medium.videoDuration = saved
                            }
                        }
                    }

                    // 3. Insere/atualiza no BD (duração já preservada)
