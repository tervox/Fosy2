...
class MediaFetcher(val context: Context) {
    ...
    /** Fallback para vídeos que o MediaStore ainda não indexou corretamente (duração = 0).
     *  Persiste o resultado no BD para não precisar recalcular na próxima abertura. */
    private fun getDurationViaRetriever(path: String): Int {
        return try {
            ...
            if (durationSec > 0) {
                try { context.mediaDB.updateVideoDuration(path, durationSec) } catch (_: Exception) {}
            }
            durationSec
        } catch (_: Exception) { 0 }
    }
        ...
        context.queryCursor(uri, projection) { cursor ->
            ...
            try {
                ...
                val videoDuration = if (isVideo) {
                    Math.round(cursor.getIntValue(MediaStore.MediaColumns.DURATION) / 1000.toDouble()).toInt()
                } else 0
                val isFavorite = favoritePaths.contains(path)
                val medium =
