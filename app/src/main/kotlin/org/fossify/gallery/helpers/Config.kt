...
class Config(context: Context) : BaseConfig(context) {
    ...
    var stretchVideo: Boolean
        get() = prefs.getBoolean(STRETCH_VIDEO, false)
        set(stretchVideo) = prefs.edit().putBoolean(STRETCH_VIDEO, stretchVideo).apply()

    var showFolderSize: Boolean
        get() = prefs.getBoolean(SHOW_FOLDER_SIZE, true)
        set(value) = prefs.edit().putBoolean(SHOW_FOLDER_SIZE, value).apply()

    fun getCachedFolderSize(path: String): String =
        prefs.getString("$FOLDER_SIZE_PREFIX${path.hashCode()}", "") ?: ""

    fun setCachedFolderSize(path: String, sizeStr: String) =
        prefs.edit().putString("$FOLDER_SIZE_PREFIX${path.hashCode()}", sizeStr).apply()
