...
class VideoFragment : ViewPagerFragment(), TextureView.SurfaceTextureListener,
    ...
    private var mIsSpeedToggled = false
    ...
    private val mTouchHoldRunnable = Runnable {
        ...
        if (mIsSpeedToggled) {
            // Já está em 2x — volta à velocidade original
            mIsSpeedToggled = false
            updatePlaybackSpeed(mOriginalPlaybackSpeed)
            mPlaybackSpeedPill.fadeIn()
            mTimerHandler.postDelayed({ mPlaybackSpeedPill.fadeOut() }, 2000L)
        } else {
            // Ativa 2x — some após 2 segundos mas mantém a velocidade
            mOriginalPlaybackSpeed = mExoPlayer?.playbackParameters?.speed ?: mConfig.playbackSpeed
            mIsSpeedToggled = true
    ...
    override fun onCreateView(
        ...
        binding = PagerVideoItemBinding.inflate(inflater, container, false).apply {
            ...
            bottomVideoTimeHolder.videoToggleStretch.setOnClickListener {
                toggleStretchVideo()
            }

            val stretchIcon = if (mConfig.stretchVideo) {
                org.fossify.commons.R.drawable.ic_cross_vector
            } else {
                R.drawable.ic_maximize_vector
            }
            bottomVideoTimeHolder.videoToggleStretch.setImageResource(stretchIcon)
    ...
    private fun initExoPlayer() {
        ...
        // Build URI directly — avoids NullPointerException from fileDataSource.uri!! after close()
        val uri = if (mMedium.path.startsWith("content://")) {
        ...
        val renderersFactory = DefaultRenderersFactory(requireContext()).setExtensionRendererMode(DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER).setEnableDecoderFallback(true)
    ...
    override fun fullscreenToggled(isFullscreen: Boolean) {
        ...
        arrayOf(
            ...
            binding.bottomVideoTimeHolder.videoToggleStretch
        ).forEach {
    ...
    fun playVideo() {
        ...
        if (!mWasVideoStarted) {
            ...
            binding.bottomVideoTimeHolder.videoToggleStretch.beVisible()
    ...
    private fun toggleStretchVideo() {
        mConfig.stretchVideo = !mConfig.stretchVideo
        setVideoSize()
        val iconRes = if (mConfig.stretchVideo) {
            org.fossify.commons.R.drawable.ic_cross_vector
        } else {
            R.drawable.ic_maximize_vector
        }
        binding.bottomVideoTimeHolder.videoToggleStretch.setImageResource(iconRes)
    }
    ...
    private fun stopHoldSpeedMultiplierGesture() {
        if (mIsLongPressActive) {
            ...
            // Não restaura velocidade aqui — o toggle mantém 2x até o próximo long press
        }
    }
}
